...
namespace Rcw.Oracle
{
    ...
    /// </summary>
    public class SqlRcwClient : SqlHelper
    {
        #region constructor
        /// <summary>
        /// 初始化 SqlRcwClient 类的新实例
        /// </summary>
        /// <param name="connectionString">数据库连接字符串</param>
        public SqlRcwClient(string connectionString)
        ...
        internal List<KeyValue> _mappingColumns
        {
            get
            {
                string cacheKey = "SqlRcwClient.InitAttributes";
                ...
                if (!_mappingTableList.Contains(mappingInfo.TableMaping))
                {
                    ...
                    if (mappingInfo.ColumnsMapping.IsValuable())
                    {
                        foreach (var item in mappingInfo.ColumnsMapping)
                        {
                            ...
                            {
                                string throwMessage = string.Format(PubModel.SqlRcwClientConst.AttrMappingError,
                                    item.Key,
        ...
        public List<T> SqlQuery<T>(string sql, List<OracleParameter> pars)
        {
            ...
            {
                sql = string.Format(PubModel.SqlRcwClientConst.SqlQuerySqlTemplate, type.Name, sql);
            }
        ...
        {
            InitAttributes<T>();
            if (rowObj == null) { throw new ArgumentNullException("SqlRcwClient.Update.rowObj"); }
            if (expression == null) { throw new ArgumentNullException("SqlRcwClient.Update.expression"); }
        ...
        public bool SqlBulkReplace<T>(List<T> entities) where T : class
        {
           ...
           return !UpdateRange(entities).Any(it=>it==false);
        }
        ...
        public bool Delete<T>(T deleteObj)
        {
            ...
            Type type = typeof(T);
            if (deleteObj == null) { throw new ArgumentNullException("SqlRcwClient.Delete.deleteObj"); }
            string typeName = type.Name;
            typeName = GetTableNameByClassType(typeName);
