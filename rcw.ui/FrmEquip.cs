...
namespace Rcw.UI
{
    public partial class FrmEquip : Form
    {
        ...
        List<TS_EQUIPMENT> deptItemList = null;
        public FrmEquip()
        {
            InitializeComponent();
        ...
        private void btn_Add_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                TS_EQUIPMENT tsDept = new TS_EQUIPMENT();
                tsDept.C_ID = PrivilegeMag.GetEquipMaxId(tag);
                tsDept.C_PARENT_ID = tag;
                tsDept.C_PARENT_NAME = PrivilegeMag.GetEquipParentFullName(tag);
                tsDept.C_NAME = "设备";
        ...
        private void btn_Del_Click(object sender, EventArgs e)
        {
            try
            {
                var item = gvEquip.GetFocusedRow() as TS_EQUIPMENT;
        ...
        private void treeView1_AfterSelect(object sender, TreeViewEventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(treeView1.SelectedNode.Name) && !string.IsNullOrEmpty(treeView1.SelectedNode.Tag.ToString()))
                {
                    ...
                    deptItemList = TS_EQUIPMENT.GetList("C_PARENT_ID=@C_PARENT_ID order by C_ID", tag);
                    gcEquip.DataSource = deptItemList;
                    gvEquip.BestFitColumns();
