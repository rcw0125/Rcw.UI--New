...
namespace Rcw.Data
{
    ...
    internal partial class ResolveExpress
    {
        ...
        private string MethodCallExpression(Expression exp, ref MemberType type, bool isTure)
        {
            ...
            {
                return CompareTo(methodName, mce);
            }
            else
            {
                type = MemberType.Value;
        ...
        private string BinaryExpression(Expression exp)
        {
            ...
            #region 处理 字符串空值“”
            else if (isKeyOperValue & (right == "") && oper.Trim() == "=")
            {
                var oldLeft = AddParas(ref left, right);
                return string.Format(" ({0} = '' ) ", oldLeft);
            }
            else if (isKeyOperValue & (right == "") && oper.Trim() == "<>")
            {
                var oldLeft = AddParas(ref left, right);
            ...
            else if (isValueOperKey & (left == "") && oper.Trim() == "=")
            {
            ...
            else if (isKeyOperValue)
            {
                ...
                else
                {
                    ...
                    return string.Format(" ({0} {1} " + ParSymbol + "{2}) ", oldLeft, oper, left);
                }
            }
            else if (isValueOperKey)
            {
                ...
                else
                {
                    ...
                    return string.Format("( " + ParSymbol + "{0} {1} {2} )", right, oper, oldRight);
                }
            }
